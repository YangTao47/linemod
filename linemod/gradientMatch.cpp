...
void lineModMatch::image_path(string path1, string path2, string path3) {
	infoSave = path1;
	trainImage = path2;
	testImage = path3;
}

void lineModMatch::scale_test(string mode, int num_feature, int retain_nums, float low_scale_value,
							  float high_scale_value, float scale_step)
{
	...
	//    mode = "test";
	if (mode == "train") {
		Mat img = cv::imread(trainImage);
		assert(!img.empty() && "check your img path");
		shape_based_matching::shapeInfo_producer shapes(img);

		shapes.scale_range = { low_scale_value, high_scale_value };
		...
		// save templates
		detector.writeClasses(infoSave + "test_templ.yaml");

		// save infos,
		// in this simple case infos are not used
		shapes.save_infos(infos_have_templ, infoSave + "test_info.yaml");
		...
		// read templates
		ids.push_back("test");
		detector.readClasses(ids, infoSave + "%s_templ.yaml");

		Mat test_img = imread(testImage);
		...
		for (size_t i = 0; i<top5; i++) {
			...
			cv::putText(img, to_string(int(round(match.similarity))),
				Point(int(match.x + r - 10), int(match.y - 3)), FONT_HERSHEY_PLAIN, 2, color);
...
/*直接对初始模板得特征点进行旋转，不需要重新找特征点，时间消耗较少，但是不能进行多尺度变换*/
void lineModMatch::angle_test(string mode, bool use_rot, int num_feature, int retain_nums, float low_angle_value,
							  float high_angle_value, float angle_step, float scale_range)
{
	line2Dup::Detector detector(num_feature, {4, 8});

	if (mode != "test") {
		Mat img = imread(trainImage);
		assert(!img.empty() && "check your img path");
		...
		shapes.angle_range = { low_angle_value, high_angle_value };
		shapes.angle_step = angle_step;

		shapes.scale_range = { scale_range }; // support just one
		shapes.produce_infos();
		std::vector<shape_based_matching::shapeInfo_producer::Info> infos_have_templ;
		string class_id = "test";
		...
		for (auto& info : shapes.infos) {
			...
			//imwrite("D:/feature_dst.jpg", to_show);
			namedWindow("train", CV_WINDOW_NORMAL);
		...
		auto infos = shape_based_matching::shapeInfo_producer::load_infos(infoSave + "test_info.yaml");

		Mat train_img = imread(trainImage);
		Mat test_img = imread(testImage);
		assert(!test_img.empty() && "check your img path");
		...
		for (size_t i = 0; i<top5; i++) {
			...
			float c_scaled = train_img.cols / 2.0f * infos[match.template_id].scale;
			float r_scaled = train_img.rows / 2.0f * infos[match.template_id].scale;
			// scaling won't affect this, because it has been determined by warpAffine
			// cv::warpAffine(src, dst, rot_mat, src.size()); last param
			float train_img_half_width = train_img.cols / 2.0f + 100;
			float train_img_half_height = train_img.rows / 2.0f + 100;
			...
			cv::circle(img, { match.x, match.y }, 3, Scalar(255,0,0), -1);
			//cout << match.x << endl;
			//cout << match.y << endl;
			cv::putText(img, to_string(int(round(match.similarity))),
				Point(int(match.x + r_scaled - 10), int(match.y - 3)), FONT_HERSHEY_PLAIN, 2, randColor);
...
/*一张图多个Roi  NMS  可以尺度 角度一起训练 但是比较耗时 每张模板都需要找特征点*/
void lineModMatch::noise_test(string mode, int num_feature, float low_angle_value, float high_angle_value,
							  float angle_step, float low_scale_range, float high_scale_range, float scale_step)
{
	line2Dup::Detector detector(num_feature, { 4, 8 });
	...
	if (mode == "train") {
		...
		shapes.angle_range = { low_angle_value, high_angle_value };
		shapes.angle_step = angle_step;
		shapes.scale_range = { low_scale_range,high_scale_range };
		shapes.scale_step = scale_step;
		shapes.produce_infos();
		std::vector<shape_based_matching::shapeInfo_producer::Info> infos_have_templ;
		string class_id = "test";
		...
		for (auto idx : idxs) {
			...
			cv::putText(test_img, to_string(int(round(match.similarity))),
				Point(int(match.x + r - 10), int(match.y - 3)), FONT_HERSHEY_PLAIN, 2, randColor);
